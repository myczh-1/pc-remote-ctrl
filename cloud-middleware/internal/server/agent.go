...
func (s *AgentServer) Register(ctx context.Context, req *cloudpb.RegisterRequest) (*cloudpb.RegisterResponse, error) {
    ...
    if req.GetDeviceId() == "" || req.GetHomeGrpcAddr() == "" {
        return nil, errors.New("device_id and home_grpc_addr required")
    }
    ttl := time.Duration(req.GetTtlSec()) * time.Second
    if ttl <= 0 {
        ttl = 60 * time.Second
    }
    s.reg.Upsert(req.GetDeviceId(), req.GetHomeGrpcAddr(), ttl, req.GetTags())
