...
func (h *eventHub) subscribe(ids []string) (*subscriber, func()) {
    ...
    for _, id := range ids { s.ids[id] = struct{}{} }
    h.mu.Lock(); h.subs[s] = struct{}{}; h.mu.Unlock()
    cancel := func() {
        h.mu.Lock(); delete(h.subs, s); close(s.ch); h.mu.Unlock()
    }
    return s, cancel
}
...
func (h *eventHub) publish(ev *homepb.DeviceEvent) {
    ...
    for s := range h.subs {
        if len(s.ids) > 0 { if _, ok := s.ids[ev.GetDeviceId()]; !ok { continue } }
        select { case s.ch <- ev: default: /* drop if slow */ }
    }
}
...
func newSubscriptionManager(c mqtt.Client, devs *storage.Devices, hub *eventHub) *subscriptionManager {
    return &subscriptionManager{mqtt: c, devices: devs, hub: hub, subs: make(map[string]bool)}
}

func (m *subscriptionManager) initAll(ctx context.Context) {
    for _, d := range m.devices.List() {
        if strings.ToLower(string(d.Adapter.Kind)) != "mqtt" { continue }
...
func (m *subscriptionManager) subscribeDevice(ctx context.Context, d *storage.Device) {
    ...
    m.ensureSub(ctx, topics.state, 1, func(topic string, payload []byte) {
        ...
        if dev != nil {
            if dev.Shadow.Reported == nil { dev.Shadow.Reported = map[string]any{} }
            for k, v := range st.AsMap() { dev.Shadow.Reported[k] = v }
...
func deriveTopics(d *storage.Device) topicSet {
    ...
    get := func(k, def string) string {
        if d.Topics != nil {
            if v, ok := d.Topics[k]; ok && v != "" { return v }
        }
        return def
    }
...
func toStruct(payload []byte) *structpb.Struct {
    ...
    if err := json.Unmarshal(payload, &v); err == nil {
        ...
        switch vv := v.(type) {
        case map[string]any:
            st, _ := structpb.NewStruct(vv); return st
        default:
            st, _ := structpb.NewStruct(map[string]any{"value": vv}); return st
        }
    }
