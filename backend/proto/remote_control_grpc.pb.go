...
const (
	...
	ControllerService_ExecuteCommandStream_FullMethodName = "/remote_control.ControllerService/ExecuteCommandStream"
)
...
type ControllerServiceClient interface {
	...
	// 流式执行，按行返回日志
	ExecuteCommandStream(ctx context.Context, in *ExecuteCommandRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[LogLine], error)
}
...
func (c *controllerServiceClient) ExecuteCommandStream(ctx context.Context, in *ExecuteCommandRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[LogLine], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ControllerService_ServiceDesc.Streams[0], ControllerService_ExecuteCommandStream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ExecuteCommandRequest, LogLine]{ClientStream: stream}
...
type ControllerService_ExecuteCommandStreamClient = grpc.ServerStreamingClient[LogLine]
...
type ControllerServiceServer interface {
	...
	// 流式执行，按行返回日志
	ExecuteCommandStream(*ExecuteCommandRequest, grpc.ServerStreamingServer[LogLine]) error
...
func (UnimplementedControllerServiceServer) ExecuteCommandStream(*ExecuteCommandRequest, grpc.ServerStreamingServer[LogLine]) error {
...
func _ControllerService_ExecuteCommandStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ExecuteCommandRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(ControllerServiceServer).ExecuteCommandStream(m, &grpc.GenericServerStream[ExecuteCommandRequest, LogLine]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ControllerService_ExecuteCommandStreamServer = grpc.ServerStreamingServer[LogLine]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _ControllerService_ExecuteCommandStream_Handler,
			ServerStreams: true,
		},
	},
