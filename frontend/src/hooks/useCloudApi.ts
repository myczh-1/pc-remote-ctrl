...
export interface CloudConfig {
  baseUrl: string
  agentId: string // Gateway device_id，用于选择目标后端
...
export function useCloudApi(config?: Partial<CloudConfig>) {
  ...
  const startWatch = useCallback((ids?: string[]) => {
    ...
    const connect = () => {
      ...
      try {
        ...
        const call = clientRef.current.watchDevices({ deviceId: agentId, request: { ids: ids ?? [] } }, { abort: abortRef.current.signal as any })
  ...
  const deleteDevice = useCallback(async (deviceId: string) => {
    try {
      const res = await clientRef.current.deleteDevice({ deviceId: agentId, request: { deviceId } }).response
      return { ok: (res as any).ok, message: (res as any).message }
    } catch (e: any) {
      return { ok: false, error: String(e?.message ?? e) }
    }
