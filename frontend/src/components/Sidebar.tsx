...
export function Sidebar({ onToggleMode, onRefreshDevices, onConfigCloud, onToggleLogs, onAddDevice }: SidebarProps) {
  const prefersReduced = useReducedMotion();
  const transition = useMemo(() => (
    prefersReduced ? { duration: 0 } : { duration: 0.2, ease: [0.22, 1, 0.36, 1] as any }
  ), [prefersReduced]);

  const Btn: React.FC<React.PropsWithChildren<{ title: string; onClick?: () => void }>> = ({ title, onClick, children }) => (
    <button
      onClick={onClick}
      className="w-10 h-10 rounded-xl border border-black/10 bg-white/70 grid place-items-center shadow hover:bg-black/5 dark:border-white/10 dark:bg-white/[0.06]"
      title={title}
    >
      {children}
    </button>
