...
static const char* FW_VERSION = "0.1.0-led";
...
static void doProvision() {
  if (g_deviceId.isEmpty()) {
    Serial.println("Provision aborted: device_id is empty");
    notifyStatus(false, "provision", "device id missing");
    return;
  }
  ...
  if (!parseMqttUrl(g_mqttUrl, g_mqttHost, g_mqttPort)) {
    Serial.printf("MQTT URL invalid: %s\n", g_mqttUrl.c_str());
    notifyStatus(false, "mqtt", "invalid url");
    return;
  }
  Serial.printf("MQTT URL: %s host=%s port=%u\n", g_mqttUrl.c_str(), g_mqttHost.c_str(), g_mqttPort);
...
class WriteCB : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic* c) override {
    ...
    if (c->getUUID().equals(NimBLEUUID(SSID_UUID)))        { g_ssid     = String(v.c_str()); g_ssid.trim(); }
    else if (c->getUUID().equals(NimBLEUUID(PASS_UUID)))   { g_pass     = String(v.c_str()); g_pass.trim(); }
    else if (c->getUUID().equals(NimBLEUUID(MQTT_URL_UUID))){ g_mqttUrl  = String(v.c_str()); g_mqttUrl.trim(); }
    else if (c->getUUID().equals(NimBLEUUID(DEVICE_ID_UUID))){ g_deviceId= String(v.c_str()); g_deviceId.trim(); }
    else if (c->getUUID().equals(NimBLEUUID(MQTT_USER_UUID))){ g_mqttUser= String(v.c_str()); g_mqttUser.trim(); }
